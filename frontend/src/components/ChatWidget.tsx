...
// Detecta el gancho de precio y extrae el nombre del producto
const extractProductFromHook = (text: string): string | null => {
  const match = text.match(/Contamos con (.+?) ideal para/i);
  return match ? match[1].trim() : null;
};

// Construye URL de WhatsApp con el producto pre-llenado
const buildWhatsAppHook = (product: string) => {
  const msg = encodeURIComponent(
    `Hola TecniMedical, estoy interesado en: *${product}*. ¿Pueden confirmarme el precio y coordinar el envío?`
  );
...
const MessageBubble = ({ message }: { message: Message }) => {
  ...
  // Detectar si el mensaje contiene el gancho de precio
  const hookProduct = !isUser ? extractProductFromHook(message.text) : null;

  // Procesamos el texto para renderizar **negrita** de forma básica
  const renderText = (text: string) => {
    // Si hay gancho, reemplazamos "haga clic aquí" por un enlace real
    if (hookProduct) {
      const parts = text.split(/(haga clic aquí)/i);
      return parts.map((part, i) =>
        /haga clic aquí/i.test(part)
          ? <a key={i} href={buildWhatsAppHook(hookProduct)} target="_blank" rel="noopener noreferrer"
              className="underline font-bold text-brand-green hover:text-brand-green/80 transition-colors">
              haga clic aquí
            </a>
          : part
      );
  ...
  return (
    ...
    >
      ...
      >
        ...
        {hookProduct && (
          <a
            href={buildWhatsAppHook(hookProduct)}
            target="_blank"
            rel="noopener noreferrer"
            className="mt-3 flex items-center justify-center gap-2 w-full py-2 rounded-lg bg-[#25D366] hover:bg-[#20bd5a] text-white text-xs font-bold transition-all"
          >
